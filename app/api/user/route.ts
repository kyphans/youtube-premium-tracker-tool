...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!description || duration === undefined || feeConstant === undefined || !startDate || !endDate || !status) {
      return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
    }
    const result = await sql`
      INSERT INTO users (description, duration, fee_constant, start_date, end_date, status, user_email, owner_family_email, payment_notes)
      VALUES (${description}, ${duration}, ${feeConstant}, ${startDate}, ${endDate}, ${status}, ${userEmail}, ${ownerFamilyEmail}, ${paymentNotes})
      RETURNING *
    `;
