...
const UserTable: React.FC<UserTableProps> = ({
  ...
  const handleCopyMessage = (user: User) => {
    const message = `Ngày hết hạn gói Youtube là ${user.endDate}, đã trễ ${Math.abs(user.available)} ngày, số tiền cần thanh toán ít nhất là ${Math.abs(user.fees)}K`;
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {users.map((user, index) => (
              ...
              >
                ...
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{user.duration}</td>
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                  {user.available}
                  {user.available >= 30 && ` (${(user.available / 30).toFixed(1)} mo)`}
                </td>
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                  {user.available >= 0 ? 0 : user.fees}
                </td>
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{user.endDate}</td>
                <td className="px-4 py-4 whitespace-nowrap">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Are you sure you want to delete user "{user.description}"? This action cannot be undone.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
